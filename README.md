# Office Oasis · 데스크톱 어시스턴트

직장인 PC가 필요한 4가지를 트레이 아이콘 하나에:

- **디스크 청소** — 거대 파일·묵은 파일·중복·스크린샷·임시 파일·빈 폴더 자동 분류 → 휴지통
- **자주 쓰는 도구 런처** — 앱·웹·폴더·파일 타일 그리드. 시작메뉴 자동 검색
- **노트** — 여러 노트 사이드바 관리. 자동 저장. `Ctrl+Shift+N`
- **클립보드 매니저** — 최근 복사 기록 200개. `Ctrl+Shift+V`

모든 분석·저장은 본인 PC에서만. 외부 전송 없음.
